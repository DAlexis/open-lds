...
ExtractGPSData(StormProcess_tPACKEDDATA *packeddata)
{
	...
	for (cnt = 0; cnt <9; cnt++)
	{	checksum += gpsdata[cnt];	} /// May be XOR operation instead of + ?
	
    tgps.TS_valid = ((checksum&0xff) == gpsdata[10]) ? true : false; /// Fix: gpsdata[8] changed to gpsdata[10] by alexey bulatov 15 dec 2014
...
*/
{
        ...
        /*
          ...
          E_Field is not exacty in phase with H field, so Offset E Field position
	*/
...
*/
{
        ...
        double d_bearing;
        /// Alexey B.: Commented out unused variables
        ...
        /*  strike degrees = 57.3 degress/rad * strike radians  */
        if (R_XValue != 0)
        ...
        /// Alexey B.: Code below is used to generate points for some kind of display. They are not really used
...
{
    /*
     * Boltek original code if this func. Variable 'strike' not used really
    ...
    */
    ...
    strike = Capture_ConvertToStrike(capture);
	strike.valid = Capture_Valid(capture); // looks like a strike?
